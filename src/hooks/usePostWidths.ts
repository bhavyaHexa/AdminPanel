...
export const useCreateWidth = () => {
  const { postData } = useAxios();
  const queryClient = useQueryClient();

  return useMutation<Width, Error, Partial<Width>>({
    mutationFn: (data) =>
      postData(API_ROUTES.WIDTHS, data) as Promise<Width>,
    onSuccess: () => {
...
const usePostWidths = useCreateWidth;
export default usePostWidths;
